...
//	u8 sendId;
    ...
    case CAN1_CHANNEL:
    {
        ...
        {
//				sendId = tempNode->nextMsg->id.idBit.sendDeviceId;
//				recvId = tempNode->nextMsg->id.idBit.recvDeviceId;
            //2018.10.14 YQ修改，增加设备有效性判断,如果设备设备不在拓扑数据范围内，直接判断无效，不作数据解析，直接返回
//				if((sys->devices[sendId]!=NULL)&&((recvId==DOMAIN_ID)||(recvId==BROADCAST_ID)))
//				{
//						HeartBeatReceived(canChan,&channel1[sendId]);
    ...
    case CAN2_CHANNEL:
    {
        while (tempNode->nextMsg != NULL)
        {
					/*如果此时是主从情况，并且这个时候帧的主索引不是这个设备类型，那说明不属于这个设备的帧，丢弃*/
					 if(tempNode->nextMsg->id.MasteridBit.MasterslaveBit==MasterSlave&&tempNode->nextMsg->id.MasteridBit.MainIndex!=EquipmentType)
					 {
						  //删除操作
                tempNode = frameBuf;
